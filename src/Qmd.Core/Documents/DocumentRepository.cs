...
internal class DocumentRepository : IDocumentRepository
{
    ...
    {
        return this.db.Prepare("SELECT id, hash, title FROM documents WHERE collection = $1 AND path = $2 AND active = 1")
            .Get<ActiveDocumentRow>(collection, path);
    }

    public void UpdateDocumentTitle(long id, string title, string modifiedAt)
    {
        this.db.Prepare("UPDATE documents SET title = $1, modified_at = $2 WHERE id = $3")
            .Run(title, modifiedAt, id);
    }

    public void UpdateDocument(long id, string title, string hash, string modifiedAt)
    {
        this.db.Prepare("UPDATE documents SET title = $1, hash = $2, modified_at = $3 WHERE id = $4")
            .Run(title, hash, modifiedAt, id);
    }

    public void DeactivateDocument(string collection, string path)
    {
        this.db.Prepare("UPDATE documents SET active = 0 WHERE collection = $1 AND path = $2 AND active = 1")
            .Run(collection, path);
    }

    public List<string> GetActiveDocumentPaths(string collection)
    {
        var rows = this.db.Prepare("SELECT path FROM documents WHERE collection = $1 AND active = 1")
    ...
    {
        this.db.Prepare("INSERT OR IGNORE INTO content (hash, doc, created_at) VALUES ($1, $2, $3)")
